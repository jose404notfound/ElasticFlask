...
class Pueblo(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            "Superficie": str(self.Superficie if self.Superficie is not None else 0),  # Valor por defecto si es None
...
# Crear el índice en Elasticsearch si no existe, con pausa
def create_index():
...
# Indexar los datos en Elasticsearch, con modo respaldo
def index_data():
    ...
    for pueblo in pueblos:
        try:
            # Preparar cada acción de indexación
            action = {
                "_index": INDEX_NAME,
            ...
            print(f"Error al preparar documento {pueblo.SimboloQuimico}: {e}")
            continue  # Continuar con el siguiente pueblo si hay un error al preparar

    # Realizar la indexación en bloques, sin detener el proceso por errores
    try:
        success, failed = bulk(es, actions, raise_on_error=False)
        print(f"Indexados {success} documentos con éxito.")
        if failed:
            print(f"{len(failed)} documentos fallaron, pero serán ignorados.")
    except Exception as e:
        print(f"Error general en el proceso de indexación: {e}")
    ...
    body = {
        "query": {
            "multi_match": {
                ...
                "fuzziness": "AUTO",  # Habilita fuzziness para coincidencias parciales (palabras similares).
                "operator": "and",    # Varias palabras a la vez.
                "prefix_length": 0  # Máxima flexibilidad en búsquedas parciales.
            }
        }
    }
